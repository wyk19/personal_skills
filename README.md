...
│   └── superpowers/
│       ├── specs/
│       │   └── 2026-07-20-routing-enforcement-design.md
│       └── plans/
│           └── 2026-07-20-routing-enforcement.md
├── tests/
│   ├── test_audit_routing_session.py
...
```bash
python routing-codex-models/scripts/audit-routing-session.py SESSION_ID \
...
CLI 同样接受根 JSONL 路径；`--sessions-dir` 始终必填且必须指向包含所有日期目录的 Codex sessions 根目录。每次审计必须在互斥的 `--latest-turn` 与 `--turn-id TURN_ID` 中选择一个。审计器使用根会话的 `turn_context` 和 spawn call 自带的 turn ID，仅核验所选 turn；started 事件和工具输出即使没有 turn ID，也会通过所选 call ID 关联到精确的子会话 UUID。
